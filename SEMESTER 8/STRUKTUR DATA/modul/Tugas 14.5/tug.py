...
class Queue:
    ...
    def to_list(self):
        ...
        # avoid multiply address decrypted bugs
        try: self.result = [data.value for data in self.result]
        except: pass
...
class BinarySearchTree:
    ...
    def original_BFS(self):
        current_node = self.root
        queue = Queue(current_node)
        results = Queue(18); results.clear_queue()

        while queue.length > 0:
            current_node = queue.dequeue(0).value
            results.enqueue(current_node)
            if current_node.left is not None:

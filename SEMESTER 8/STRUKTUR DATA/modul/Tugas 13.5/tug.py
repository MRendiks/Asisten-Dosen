...
def pivot(my_list, pivot_index, end_index):
    ...
    pivot_compare = random.randint(0, len(my_list)-1)
    swap_index = pivot_index

    for i in range(pivot_index+1, end_index+1):
        if my_list[i] < my_list[pivot_compare]:
            swap_index += 1
...
def quick_sort(my_list):
    while not isHaveSorted(my_list):
        my_list = quick_sort_helper(my_list, 0, len(my_list)-1)
...
# change __main__ into anything if you want to avoid the runned of this one
...
def quick_sort_system(my_list):
    ...
    # declare it first
    indexNumber = -1; highest_value = my_list[0]
    leftSide = []; rightSide = []; lowest_value = my_list[0]
    pos = random.randint(0, len(my_list)-1); pivot = my_list[pos]
    # randomize the pivot index

    # put the pivot value into the -1 index
    ...
    for counter in range(len(my_list)-1):
        if(pivot < my_list[counter+1] and pivot > my_list[counter]): 
            my_list.insert(counter+1, my_list[-1])
            del my_list[-1]; break
...
#--> change __main_ into __main__ if you want to run this
if __name__ == '__main_':
    listResult = []; my_list = []
    print('============= Daftar List =============')
